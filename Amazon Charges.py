...
import pandas as pd
import numpy as np
import io  # FIXED: io is now correctly imported
...
def process_payment_files(uploaded_payment_files):
    ...
    2. df_charge_breakdown: The raw data for detailed charge view (used only for export)."""
    ...
    # --- PIVOTING: Create Financial Master Summary (FOR SCREEN DISPLAY) ---
    principal_df = df_payment_cleaned[df_payment_cleaned['amount-description'] == 'Principal'].groupby('OrderID')['amount'].sum().reset_index(name='Total_Principal')
    
    fee_descriptions = [
        'FBA Pick & Pack Fee', 'FBA Weight Handling Fee', 'Commission', 'Fixed closing fee', 
        'Technology Fee', 'Refund commission', 'FBA Pick & Pack Fee CGST', 
        'Commission IGST', 'Fixed closing fee IGST', 'Technology Fee IGST',
        'Commission CGST', 'Commission SGST', 'Fixed closing fee CGST', 'Fixed closing fee SGST'
    ]
    
    df_fees = df_payment_cleaned[df_payment_cleaned['amount-description'].isin(fee_descriptions)]
    ...
    # Returning both the summary and the raw data (raw data for Excel export later)
    return df_financial_master, df_payment_cleaned 
...
if payment_files and mtr_files:
    ...
    # Remove the detailed breakdown from the bottom, as requested
    # st.header("2. Detailed Charge Breakdown (Payment File)") -> Removed
